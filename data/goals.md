...
> 狀態：**評估中，未拍板**。演變過程（誰在什麼時候說了什麼）見 [plan-changelog.md](plan-changelog.md)，本節只寫**目前的狀態與判準**。

**專案內容**：LEX System 的瑕疵檢測專案，題材是**缺鍵、錯鍵、空焊**——屬電子構裝／打線接合（wire bonding）或 PCBA 的焊接瑕疵檢測（AOI）。為期半年（約 2026-09 → 2027-02），**每週書面報告 ＋ 期中／期末**（同學轉述，待自己確認）。**資料可以發表論文**，教授建議接。

**教授的想法是「論文相關的都用 LEX System」**，而他 7/28 才親自定名的題目是手術器械 VLA（見上一節）——**兩者不是同一件事，距上次換題目只有三週**。這是本決策的核心張力。

#### 三個必須先問清楚的問題

教授「好像也沒有要接機器人的案子」，也未必是 LEX 這個計畫的主持人。若是如此，「資料可發表、畢業比較簡單」這個主要理由就站不穩：

1. **「資料可以發表」是誰保證的？** 授權由誰批——業主？計畫主持人？
2. **誰指導？** 教授不接案子的話，技術問題找誰
3. **這個計畫的正式身分是什麼？**（學校計畫／公司直接找學生／補助案）掛名怎麼算

另外要問教授的三句話：7/28 的手術器械 VLA 題目**還算不算數**？若兩者都要，**哪個是主體**？LEX 的資料**能不能寫進論文、能不能發表**？

#### 與論文的重疊度：只剩方法論

原本設想「一個方法、兩個場域」（LEX 工業瑕疵 ＋ CSSD 器械檢查），**但題材確認後這個判斷要下修**：

- 缺鍵／錯鍵／空焊是電子構裝瑕疵，**不是金屬件表面缺陷**；影像尺度與成像方式都不同（**空焊多半要 X-ray 或 3D SPI，不是一般 RGB**）→ **資料、模型、場域都無法共用**
- **仍可共用的只有方法論**：「**用合成資料解決缺陷樣本稀少**」在 AOI 同樣是真痛點（良品極多、缺陷極少），而這正是使用者最強、市場上相對稀缺的能力（Replicator，已做到 mAP50 ~0.9）
- ⇒ 強度從「一個系統兩個場域」下修為「**同一套方法論，兩篇各自獨立的實作**」
...
| 教授指導 | **實質**（影像檢測是他的專長） | 形式掛名為主——他不接機器人案子 |
| 資源 | 教授／業主的專案 | **ITRI 已在手，已投入約 3 個月** |
| **職涯對口（T1 具身智能）** | **弱**——AOI 對 T1 幫助有限，只對 T2 機器人視覺有用 | **強**——正是達明／台達／Solomon 在做的事 |
| 主管需求 | 無關 | ITRI 主管明說缺 chatbox／VLA |
| 已完成的資產 | 方法可移植、資料不可移植 | 教授步驟 1–3 已完成 |

**「畢業比較簡單」是正當理由，不是偷懶**：上一個 RL 題目就是死在資料不可行，**「資料確定可用、可發表」正好對症**，這一項權重要給高。但也要正視代價——**AOI 論文對職涯 T1 的說話能力弱**。

#### 決定這件事的關鍵問題

**系所的畢業門檻是否要求論文發表？**

- **要求** → LEX 正好補上「一篇可發表的論文」，**論文主體仍可留在 ITRI**，不必為了發表整本改題。成本最低的組合。
- **不要求** → 才是單純二選一：畢業穩（A）vs 職涯對口（B）。

#### 時程與 ITRI 的定位（合約年底自然到期）

**ITRI 實習合約本來就是 2026-12 到期，薪資不是考量**（使用者：「離職還好，錢沒差」）⇒ **沒有「要不要離職」的問題，只有「到期後要不要續」**。時程天然對齊：**ITRI 9–12 月（4 個月，有明確結束日）／ LEX 2026-09 → 2027-02**。

**⚠️ 順序不能顛倒**：`LEX 三題有答案` → `論文重心轉移` → `ITRI 到期自然結束`。在 LEX 落地之前，不要先把 ITRI 這條線收掉。

**9–12 月 ITRI 的目標**：不追求做完教授 10 步驟的全部，收斂成**一個可展示的段落**——**辨識 → 抓取 → chatbox 下指令**的最小可行 demo ＋ 錄影 ＋ README ＋ 數據。這正是主管說缺的 chatbox／VLA，對 ITRI 有交代、對自己是作品集，而且不必等 G1 到貨。

**年底檢查點（2026-11 底判斷要不要續約）**：

| 檢查項目 | 到期結束、專心 LEX | 爭取續約 |
|---|---|---|
| LEX 資料到手、教授實質指導、有發表進度 | 是 | 否 |
| **G1 到貨了嗎**（權重最高） | 沒到 | **到了 → 續**（實機人形經驗難得，教授第 10 項才做得完） |
| ITRI 分工明確、chatbox 有進展 | 沒有 | 有 |
| 哪邊論文素材紮實 | LEX | ITRI |

#### 待辦

- ⬜ 問清楚上面**三個問題 ＋ 三句話**，以及**系所畢業門檻是否要求發表**
- ⬜ 拿到 LEX 正式說明後確認：資料量與瑕疵種類、**是否已標註**、有沒有 baseline 可比、目標投稿層級、是否需進駐、有無酬勞掛名、期中期末會不會撞論文里程碑
- ⬜ 確認**到期後還能不能用 ITRI 的資料與成果寫論文／放作品集**（授權問題，與 LEX 同類。不能用的話，這四個月的投入強度要重新評估）
- ⬜ **無論走哪條**：到期前把 ITRI 成果收成作品集（合成資料 pipeline、YOLO 辨識、RMPflow reach demo、chatbox 最小可行版 → demo 影片＋README＋數據）。這是職涯 T1 的核心資產，不能因合約到期就散掉
- ⬜ 決定後：主動跟 ITRI 主管說明論文與實習的關係，避免兩邊期待錯位

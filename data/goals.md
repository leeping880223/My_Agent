...
### ✅ 已定案：論文主體＝ITRI，LEX＝履歷專案（2026-08-21 拍板）

| | 定位 | 期間 |
|---|---|---|
| **ITRI 手術器械 ＋ VLA／chatbox** | **畢業論文主體**（也是研討會論文的來源） | 實習 2026-09 → 12（合約到期）；論文寫作延續到 2027 上半年 |
| **LEX System 瑕疵檢測** | **履歷專案**，不是論文 | 2027-01 → 06 |

**LEX 專案**：缺鍵、錯鍵、空焊（電子構裝／打線接合或 PCBA 的焊接瑕疵檢測，AOI）。計畫主持人就是 Basanta 教授，每週書面報告 ＋ 期中／期末。與論文題材**當作不重疊**，能帶過去的只有經驗與方法（尤其「用合成資料補缺陷樣本」在 AOI 同樣是真痛點）。

**為什麼不讓 LEX 當論文主體**：它 2027-01 才開始、2027-06 結束，與 2027 年中口試幾乎同時；而 ITRI 是**現在就能做研究的唯一窗口**，資源在手、主管有明確需求（chatbox／VLA），且**職涯 T1 對口**（達明／台達／Solomon）。

### 畢業門檻與時程

**門檻**：**研討會論文一篇 ＋ 完整的研究論文（不必投稿）**。畢業時間 **2027 年中**。

⇒ 門檻不要求期刊接受，**研討會論文的審查快、時程壓力小，ITRI 的成果完全撐得起來**。時程排法：

```
2026-09 ── 12   ITRI 實習：做出 chatbox demo ＋ 實驗數據      ← 唯一的研究窗口
2027-01 ── 03   投研討會論文 ／ LEX 開跑
2027-03 ── 06   學位論文寫作 ＋ LEX 進行 ＋ 口試
```

**⬜ 最該先問掉的一件事：目標研討會與投稿 deadline。** deadline 會倒推決定 9–12 月的成果必須在哪一週前做完。教授通常有慣用的研討會，直接問他最快。

#### 待辦

- ⬜ **問教授：目標研討會與投稿 deadline**（倒推 9–12 月的里程碑）
- ⬜ 跟教授確認：論文主體留在 ITRI、LEX 當專案，他是否同意這個分工；7/28 的手術器械 VLA 題目是否維持
- ⬜ 確認**實習到期後還能不能用 ITRI 的資料與成果寫論文／放作品集**（不能用的話，整個計畫要重排）
- ⬜ 拿到 LEX 正式說明後確認：資料量與瑕疵種類、是否已標註、有無 baseline、是否需進駐、有無酬勞掛名
- ⬜ 主動跟 ITRI 主管說明論文與實習的關係，避免期待錯位
- ⬜ 到期前把 ITRI 成果收成作品集（demo 影片＋README＋數據）——職涯 T1 的核心資產

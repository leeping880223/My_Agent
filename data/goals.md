...
3. **ITRI 的實體支援不存在**——該部門現在在**台南六甲**辦公，使用者全程遠距且明確不去。所以 ITRI 這條線＝純模擬、無實機、無現場協助。**撐得起一篇研討會論文，撐不起一本論文的實驗章節。**
...
| **工研院（ITRI）** | 推 **VLA 應用**：可下指令，**未來同一個指令可以下給多台機器人**；另外想要人形會走路 | 使用者的觀察：**走路目前沒有好的應用場景**。⚠️ 更關鍵的是**該部門在台南六甲辦公，使用者全程遠距且明確不去**——這條線只能做模擬 |
...
**前提**：ITRI 該部門在**台南六甲**辦公，使用者全程**遠距**。所以這 13 週**純模擬、無實機、無現場協助**，任何需要有人在機台旁邊的事都排不進來。
...
| 階段五 自主操作與多機協作 | ⬜ 移出主線 |
| 階段六 Real-World Validation | ❌ **不可行**——ITRI 部門在台南六甲、使用者遠距，且 G1 採購本來就沒把握 |
...
### 階段六｜步驟 10：Real-World Validation（**❌ 不可行，保留供翻盤時參考**）
> ❌ **這一段實際上做不到**：ITRI 部門在台南六甲、使用者全程遠距；且工研院說要買 G1 但沒把握。**論文主體因此不能押在 ITRI**——這是方案轉向 LEX 的第 3 條理由。若真要做實機驗證，只剩 Plan B（用實驗室現有手臂做局部驗證）。

...
⚠️ **換機器人的成本**：模擬與實機要用同一台，否則 sim-to-real 對不起來。手刻過的 G1 URDF ＋ RMPflow 要重做，而**工研院不給 code**。**但這正是 PoC 架構要兌現的地方**——通用層（辨識／姿態／抓取規劃／指令解析）全部保留，只換平台相依層。
...
#### 9/1 之前要問 ITRI（用問的，不要啟動合作流程）

⚠️ **「問資訊」和「承諾合作」是兩件事。** 以實習生身分問專案安排是本分，不需要理由也不需要包裝；**啟動合作流程才是承諾，那要選定之後再做。**

1. **分工會議什麼時候？我能不能提案接 chatbox／VLA／BEV 這塊？**
2. ⭐ **那台雙臂機器人的規格**：有沒有手腕相機（BEV 要用）？夾爪夾不夾得起細長器械？控制介面開不開放？
3. **我遠距 ＋ 偶爾去台南，實際碰得到那台嗎**？它是展場 demo 機還是實驗室常駐？
4. **台南實習的實際安排**（頻率、時長）、**發表授權的範圍**
...
⚠️ **教授的判準是「有沒有現成 dataset、train 得快、不花太多時間」。** 所以提合成資料時他很可能問「有沒有現成資料集」——**答案要現成**：AOI 的公開資料集極少，且都不是目標機台的光學條件，**這正是題目成立的理由，不是缺點**。
...
⚠️ **代價**：這是另一個題目，不是免費附加。**13 週塞不下「chatbox v1 ＋ 量化評估 ＋ BEV」**，要選：BEV 當感知底層（chatbox 簡化），或 chatbox 先做完（BEV 當延伸）。
⬜ 待確認：ITRI 那台雙臂機器人有沒有手腕相機。

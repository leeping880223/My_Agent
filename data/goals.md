...
1. **指導品質**——Basanta 是 LEX 的計畫主持人，而他**確實有在指導**：建議有用、用心，會主動推學生去比賽（碩一下兩個競賽都是他推薦，拿了第一名與第二名），也認可使用者的報告能力與切入角度。相較之下，機器人那層他無法判斷「做到哪算夠」，也無力在口試替你辯護。
   ⚠️ **但他的可靠範圍有邊界**：方向、機會、鼓勵可靠；**技術可行性的把關不可靠**（見閘門的說明）。**所以是「有人指導」，不是「有人保證做得出來」——可行性仍然要自己驗。**
...
⚠️ **這道閘門存在的理由，是已經被打過兩次的地方**：① 上一個 RL 題目死於資料不可行（收完才發現 action data 是 perception data 的 4 倍以上且不規則）；② 曾有老師交代的方案**邏輯上就做不出來**，是使用者自己發現的——「要是我沒發現，我就去了，這輩子都做不出來」。**教授在方向與機會上可靠，但在技術可行性上不是**。所以下面每一項都要自己驗，不能因為「他說可以」就開工。

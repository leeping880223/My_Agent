...
**⚠️ 2026-08-21 新的不確定性：教授「好像也沒有要接案子」。**
若教授不是這個計畫的主持人，以下三件事就沒有著落，**要先問清楚再答應**：
1. **「資料可以發表」是誰保證的？** 授權由誰批——業主？計畫主持人？
2. **誰指導？** 教授不接案子的話，技術上遇到問題找誰
3. **這個計畫的正式身分是什麼？**（學校計畫／公司直接找學生／某個補助案）與掛名方式

→ 若這三題沒有明確答案，「資料可發表、畢業比較簡單」這個最主要的理由就站不穩，決策要重新評估。

#### 2026-08-21（三度更新）：題材確認為電子構裝瑕疵，交集比原先估計的弱

- **LEX 的題材（使用者轉述）**：**缺鍵、錯鍵、空焊**——屬**電子構裝／打線接合（wire bonding）或 PCBA 的焊接瑕疵檢測（AOI）**，不是金屬件表面缺陷。
- **⚠️ 這推翻了先前的判斷**：稍早設的判準是「若瑕疵屬金屬件表面（跟器械鏽蝕、刮痕同源）則共用成立」——**答案是否定的**。
  - 影像尺度、成像方式都不同（**空焊多半要 X-ray 或 3D SPI，不是一般 RGB**），**資料、模型、場域都無法共用**。
  - **仍可共用的只剩方法論**：「**用合成資料解決缺陷樣本稀少**」在 AOI 同樣是真痛點（良品極多、缺陷極少）。→ 共用強度從「一個方法、兩個場域」下修為「**同一個方法論，兩篇各自獨立的實作**」。
- **教授的立場（使用者判斷）**：**教授沒有要接機器人的案子，硬要他接會很麻煩**；但**使用者可以用 ITRI 的資源做機器人題目，之後發表論文應該可行**。
  - 意思是：機器人方向**教授不擋、但也不會實質指導**（他的專長是影像檢測，正好對得上 LEX，對不上人形與模擬）。

#### 兩條路的取捨（2026-08-21）

| | **A. 論文做 LEX（電子構裝瑕疵檢測）** | **B. 論文做 ITRI（手術器械 ＋ VLA）** |
|---|---|---|
| 畢業風險 | **低**（資料到位、教授能實質指導） | 中（G1 到貨沒把握、指導真空） |
| 教授指導 | **實質**（他的專長） | 形式上掛名為主 |
| 資源 | 教授／業主的專案 | **ITRI 已在手，已投入約 3 個月** |
| **職涯對口（T1 具身智能）** | **弱**——AOI 檢測對 T1 幫助有限，只對 T2 機器人視覺有幫助 | **強**——正是達明／台達／Solomon 在做的事 |
| 主管需求 | 無關 | ITRI 主管明說缺 chatbox／VLA |
| 已完成的資產 | 需重做（合成資料 pipeline 可移植方法但不可移植資料） | 步驟 1–3 已完成 |

**⬜ 決定這件事的關鍵問題：系所的畢業門檻是否要求論文發表？**
- **若是**：LEX 正好補上「一篇可發表的論文」，**論文主體仍可留在 ITRI**——不必為了發表而整本改題。這是成本最低的組合。
- **若否**：那就是單純的二選一，取捨在「畢業穩（A）」與「職涯對口（B）」之間。

**目前傾向**：**主體 B ＋ 用 A 換一篇發表**，並讓兩者共用「合成資料補缺陷樣本」這條方法論。若 ITRI 半年內卡死（G1 沒到、分工沒下來），再把重心移到 A。

#### 選項 C：重心放 LEX，ITRI 實習**年底自然到期**（2026-08-21 釐清）

> **2026-08-21 使用者確認：實習合約本來就是年底到期，且薪資不是考量（「離職還好，錢沒差」）。**
> ⇒ **根本沒有「要不要離職」這個問題**，只有「**到期後要不要續**」。決策的心理成本與財務成本都歸零，剩下唯一的問題是：**9–12 月這四個月，ITRI 要投入到什麼程度、產出什麼。**
> ⇒ 也代表時程天然對齊：**ITRI 9–12 月（4 個月，有明確結束日）／ LEX 半年計畫 2026-09 → 2027-02**。

**這個選項的價值在於它不是立刻二選一，而是保留 4 個月緩衝**（2026-09 → 12），時間上也剛好接得起來：LEX 半年計畫約 2026-09 → 2027-02，離職後正好全力衝期末。

**⚠️ 順序仍不能顛倒**：**先確認 LEX 真的落地，再把論文重心移過去。** 目前 LEX 還有三個未解問題（誰保證資料可發表／誰指導／計畫的正式身分）。在那之前不要先把 ITRI 這條線收掉。

正確順序：`LEX 三題有答案` → `論文重心轉移` → `ITRI 到期自然結束`

**9–12 月 ITRI 的目標（有結束日的衝刺，不是無限期投入）**：
- **不**追求完成教授 10 步驟的全部
- 目標收斂成一個**可展示的段落**：辨識 → 抓取 → **chatbox 下指令** 的最小可行 demo ＋ 錄影 ＋ README ＋ 數據
- 這正好是**實習主管說缺的 chatbox／VLA**——對 ITRI 有交代，對自己是作品集
- 12 月底自然結束，不欠人情，也不半途而廢

**年底檢查點（2026-11 底判斷要不要爭取續約）**：

| 檢查項目 | 到期結束、專心 LEX | 爭取續約 |
|---|---|---|
| LEX 資料到手、教授實質指導、有發表進度 | 是 | 否 |
| **G1 到貨了嗎**（權重最高） | 沒到 | **到了 → 續**（實機人形經驗難得，教授第 10 項也才做得完） |
| ITRI 分工明確了嗎、chatbox 有進展嗎 | 沒有 | 有 |
| 哪邊的論文素材較紮實 | LEX | ITRI |

**到期前一定要做的事（無論後續走哪條）**：**把 ITRI 的成果收尾成可展示的作品集**——合成資料 pipeline、YOLO 辨識、RMPflow reach demo、chatbox 最小可行版，整理成 demo 影片＋README＋數據。**這是職涯 T1 的核心資產**（達明／台達／Solomon 面試時要拿出來的就是這些），**不能因為合約到期就散掉**。

**⬜ 唯一還要確認的一件事**：**到期後還能不能用 ITRI 的資料與成果寫論文／放作品集？**（授權問題，與 LEX 是同一類。若不能用，這四個月就只剩履歷一行，投入強度要重新評估。）

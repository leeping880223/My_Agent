...
├── todo.md                  # 其他待辦（非論文專案的雜項），做完即刪，不留歷史
...
使用者說要**換聊天室／開新對話／重開**（或表示這個對話太長了）時，**主動執行下列檢查並回報結果**，不要只回「好」。新對話沒有本次對話的脈絡，只讀得到硬碟上的東西。

1. **掃出「只存在對話裡」的內容**，逐一補寫到對應檔案：
   - skill 規則的改動 → 對應的 `SKILL.md`
   - 踩坑、發現不可行的方法 → `data/dev-log/`
   - 任務感受 → `data/profile/task-feelings.md`
   - 專案決策與轉向 → `data/goals.md`、`data/plan-changelog.md`
   - 跨 session 的偏好、專案背景 → 長期記憶
   - ⚠️ **最容易漏的兩類**：口頭說「心裡有個底」「之後再處理」的事；以及 agent 自己提出、使用者點頭但沒人寫下的待辦。
2. **未完成的工作**：把「目前進行到哪、下一步是什麼」寫進對應檔案，不要只留在對話裡。
3. **git 狀態**：`git status` 確認全部已 commit 並 push。
4. **CLAUDE.md 本身**：新對話只靠這份檔案當入口，確認本次的流程改動都已反映在此。
5. **給使用者一句開場白**：明確告訴使用者新對話可以怎麼開場、從哪裡接續。
